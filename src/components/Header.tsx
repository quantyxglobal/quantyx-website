        ...
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8">
            ...
            <div className="relative" ref={globalRef}>
              <button
                className="flex items-center gap-1 text-foreground/80 hover:text-primary transition-all font-medium"
                onClick={() => { setGlobalDeliveryOpen(!globalDeliveryOpen); setWhyTrustOpen(false); }}
              >
            ...
            <div className="relative" ref={trustRef}>
              <button
                className="flex items-center gap-1 text-foreground/80 hover:text-primary transition-all font-medium"
                onClick={() => { setWhyTrustOpen(!whyTrustOpen); setGlobalDeliveryOpen(false); }}
              >
                <Shield className="w-4 h-4" />
                Why Trust Us
                <ChevronDown className={`w-4 h-4 transition-transform ${whyTrustOpen ? "rotate-180" : ""}`} />
              </button>
              {whyTrustOpen && (
                <div className="absolute top-full left-0 mt-2 w-56 bg-background border border-border rounded-xl shadow-lg py-2 z-50">
                  <Link
                    to="/hipaa"
                    className="flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-primary/5 hover:text-primary transition-colors"
                    onClick={() => setWhyTrustOpen(false)}
                  >
                    <Shield className="w-4 h-4 text-primary" /> HIPAA & Security
                  </Link>
                  <Link
                    to="/hipaa-pipeda-compliance"
                    className="flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-primary/5 hover:text-primary transition-colors"
                    onClick={() => setWhyTrustOpen(false)}
                  >
                    <Shield className="w-4 h-4 text-primary" /> HIPAA & PIPEDA
                  </Link>
                  <Link
                    to="/pricing"
                    className="flex items-center gap-3 px-4 py-2.5 text-sm hover:bg-primary/5 hover:text-primary transition-colors"
                    onClick={() => setWhyTrustOpen(false)}
                  >
                    <span className="text-primary font-bold text-sm">$</span> How Pricing Works
                  </Link>
                </div>
              )}
            </div>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden mt-4 pb-4 border-t border-primary/10 pt-4">
            <nav className="flex flex-col gap-4">
              ...
              <div>
                <p className="text-xs font-semibold text-primary uppercase tracking-wider mb-2 flex items-center gap-1">
                  <Shield className="w-3 h-3" /> Why Trust Us
                </p>
                <div className="pl-4 flex flex-col gap-2">
                  <Link to="/hipaa" className="text-foreground/80 hover:text-primary text-sm" onClick={() => setMobileMenuOpen(false)}>HIPAA & Security</Link>
                  <Link to="/hipaa-pipeda-compliance" className="text-foreground/80 hover:text-primary text-sm" onClick={() => setMobileMenuOpen(false)}>HIPAA & PIPEDA</Link>
                  <Link to="/pricing" className="text-foreground/80 hover:text-primary text-sm" onClick={() => setMobileMenuOpen(false)}>How Pricing Works</Link>
                </div>
              </div>
